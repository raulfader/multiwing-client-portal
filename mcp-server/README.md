...
| `MULTIWING_ADMIN_EMAIL` + `MULTIWING_ADMIN_PASSWORD` | one of the two auth options (**not recommended for live yet**) | The admin login the *serving* process loaded. Values from the Manus secrets panel have been rejected by live, and the Manus editor reached from the project card isn't the one serving the domain (see [AUDIT.md](AUDIT.md#manus-console-observation-reported-by-raul-read-only-2026-09-25)). `whoami` warns when this is the only credential. The server logs in on first use and again when the session expires, and logs out on exit. If set together with a session token, it is used as the fallback when the token expires. |
